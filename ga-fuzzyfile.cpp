...
void readfile(int qwer, int seed) {
...
Individual output(Individual ob) {
    ...
    ob.tdichuyen=dichuyen;
    ob.tsac=sac;
    ob.fitness=sonutchet+abs(Pm*dichuyen)/Emc;
    return ob;
}
...
int main() { 
    // ten file chay se duoc luu trong mang tenfile[], file he so sac chay tuong ung se luu trong mang hesosac[]. Phan tu dau tien la tenfile[1] và hesosac[1]
    DIR *dpdf;
    struct dirent *epdf;
    dpdf=opendir("dulieu");
    int ks=0,i=1;
    ...
    string fileghi2="Fuzzy-GA/thoigiandichuyenSeed=1-10.txt";
    string fileghi3="Fuzzy-GA/canduoi.txt";
	...
	double E[4]={13500, 27000, 54000, 81000};
	    ...
	    for(int i=1;i<=5;i++) {
		    ...
		    for(int seed=1;seed<=1;seed++) {
                ...
                for(int itt=1;itt<=n;itt++) {
            	    if((sensors[itt].energy-Emin)/sensors[itt].p>=T) sensors[itt].xacsuatsac=beta+(1-beta)*sensors[itt].hesosac/5.0;
            	    else {
            		    if(sensors[itt].energy+(Emax-Emin)-sensors[itt].p*T<Emin) {sensors[itt].xacsuatsac=gamma*sensors[itt].hesosac;canduoi++;}
